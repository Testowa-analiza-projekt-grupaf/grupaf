...
```{r german-credit, echo = FALSE}
# czy my mamy tutaj tylko purpose wsatwiać,czy może weżmiemy te zmienne które pojawiają się na wykresach niżej (account_balance, age, previous credits, credits at this bank, employment)
german_credit %>% 
  bind_shadow() %>% 
...
Dobry_klient <- data.frame(Wiek = "<= 55", Zaciągnięte_kredyty_w_danym_banku= "<= 2", Środki_na_koncie= ">= 200 DM", Zatrudnienie = ">= 3", Wcześniej_zaciągnięte_kredyty = "NIE WIEM")
Zły_klient <- data.frame(Wiek = "> 55", Zaciągnięte_kredyty_w_danym_banku= "> 2", Środki_na_koncie= "< 0 DM", Zatrudnienie = "< 3", Wcześniej_zaciągnięte_kredyty = "NIE WIEM")
...
```{r}
tabela_klientow_transpose <- rbind(Dobry_klient_transpose, Zły_klient_transpose)
kable(tabela_klientow_transpose, col.names = c("Dobry klient", "Zły klient"), align="cc", caption = "Tabela 2. Porównywanie cech dobrego i złego klienta")
```
